...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		schoolBG = new BGSprite('weebEvil/animatedEvilSchool', 0, 0, null, null, ['bgAnimated'], true, 35);
		schoolBG.setGraphicSize(FlxG.width + 10, FlxG.height + 10);
	...
	override function update(elapsed:Float)
	{
		...
		if (!FlxG.keys.justPressed.LEFT || !FlxG.keys.justPressed.A) {
			FlxTween.tween(leftArrow, {alpha: 0.3}, 0.2, {
				startDelay: 0.1
			});
		}
		if (!FlxG.keys.justPressed.RIGHT || !FlxG.keys.justPressed.D) {
			FlxTween.tween(rightArrow, {alpha: 0.3}, 0.2, {
				startDelay: 0.1
			});
		}
